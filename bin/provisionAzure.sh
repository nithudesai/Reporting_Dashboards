...
#echo private_key_file_PWD=$CDOPS_PRIVATE_KEY_PWD >> ./resource/snowflake.properties
JAVA_CMD=""
if [ -z ${JAVA_HOME+""} ];
then
...
./liquibase/liquibase --defaults-file=./resource/liquibase.properties --changeLogFile=./masterchangelog.xml --log-level=info update -DTASK_WAREHOUSE=$TASK_WAREHOUSE -DACCOUNT_LOCATOR=$ACCOUNT_LOCATOR -DORGANIZATION=$ORGANIZATION
